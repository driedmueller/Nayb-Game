...
namespace Engine
{
    public class Player : LivingCreature
    {
        ...
        public bool HasAllQuestCompletionItems(Quest quest)
        {
            ...
            {
                // Check each item in player's inv, to see if they have, and have enough
                if (!Inventory.Any(ii => ii.Details.ID == qci.Details.ID &&
                     ii.Quantity >= qci.Quantity))
                {
                    return false;
                }
            }
            // If we got here, then the player must have all required items, and enough of them, to complete quest
            return true;
        }
        ...
        public void RemoveItemFromInventory(Item itemToRemove, int quantity = 1)
        {
            ...
            if (item == null)
            {
                // Item is not in the player's inv so ignore it
                // We might want to raise an error for this situation
            }
            else
            {
                // They have the item so decrease the quantity
                ...
                // If the quantity is zero, remove the item from list
                if (item.Quantity == 0)
                {
                    Inventory.Remove(item);
                }

                //Notify the UI that inv has changed
        ...
        public void RemoveQuestCompletionItems(Quest quest)
        {
            foreach (QuestCompletionItem qci in quest.QuestCompletionItems)
            {
                // subtract the quantity from the player's inv that was needed to complete the quest
        ...
        public void MarkQuestCompleted(Quest quest)
        {
            ...
            PlayerQuest playerQuest = Quests.SingleOrDefault(pq =>
                pq.Details.ID == quest.ID);
        ...
        {
            MoveTo(World.LocationByID(World.LOCATION_ID_CAMPSITE));
        }
